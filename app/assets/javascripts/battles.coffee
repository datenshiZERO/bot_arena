...
updateUnitInfoBox = (unit, isCurrent) ->
  ...
  $("#{id} .other-info").append("&nbsp; #{unit.move} <span class='fa fa-arrows'></span> #{unit.defense}/#{unit.evade} <span class='fa fa-shield'></span> #{unit.damage}/#{unit.accuracy}/#{unit.range_min}-#{unit.range_max} <span class='fa fa-crosshairs'></span>")
